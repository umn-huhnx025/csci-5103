...
typedef int (*next_page_t)(struct page_table *pt);
...
struct page_table *page_table_create(int npages, int nframes,
...
The bits may be any of PROT_READ, PROT_WRITE, or PROT_EXEC logical-ored
together.
*/
...
"frame" and "bits" must be pointers to integers which will be filled with the
current values. The bits may be any of PROT_READ, PROT_WRITE, or PROT_EXEC
logical-ored together.
*/

void page_table_get_entry(struct page_table *pt, int page, int *frame,
                          int *bits);

/* Return a pointer to the start of the virtual memory associated with a page
 * table. */

char *page_table_get_virtmem(struct page_table *pt);

/* Return a pointer to the start of the physical memory associated with a page
 * table. */
